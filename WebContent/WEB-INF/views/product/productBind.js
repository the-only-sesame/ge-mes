...
//						Mustache.render(list=new ArrayList<String>(){"a01","a02"},{"name":"list[i].name","gender":list[i].gender});
						var rendered = Mustache.render(
								productBindTemplate,//<script id="planListTemplate" type="x-tmpl-mustache">
										{
...
//						Mustache.render(list=new ArrayList<String>(){"a01","a02"},{"name":"list[i].name","gender":list[i].gender});
                    ...
                    var page2Size = $("#pageSize").val();
					var page2No = $("#product2Page .pageNo").val() || 1;
					//渲染页码
					renderPage(
							url,
							result.data.total,
							...
							page2Size,
							result.data.total > 0 ? result.data.data.length : 0,
							"product2Page", loadProductBoundList);
				} else {
					showMessage("获取订单列表", result.msg, false);
				}
			}
